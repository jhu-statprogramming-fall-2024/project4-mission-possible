...
Our goal is to generate 3-year forecasts for the number of trials by country, trial distribution by disease type, and trial phase patterns. Thus, we firstly setup a general function and then separately discuss three tasks as below.
...
# Set seed for reproducibility in parallel computing
clusterSetRNGStream(cl, 123)
```

### Shared Training Function 
The models will be trained using cross-validation, and firstly a general training function should be setup since all three tasks use same structure:
```{r}
train_models <- function(target, prepared_data, category_name) {
  set.seed(123)
  # Filter data for target
  subset_data <- prepared_data[prepared_data[[category_name]] == target, ]
  subset_data <- subset_data[order(subset_data$year, subset_data$month), ]
  
  model_data <- data.frame(
    trial_count = subset_data$trial_count,
    time_index = 1:nrow(subset_data),
    month_factor = factor(subset_data$month)
  )
  ...
  # Train LASSO with parallel processing
...
country_models <- foreach(country = unique(train_count_data$country),
                         .packages = c("caret", "glmnet", "randomForest")) %dopar% {
  train_models(country, train_count_data, "country")
}
names(country_models) <- unique(train_count_data$country)

# 2. Disease Models
train_disease_data <- prepare_disease_data(train_data)
disease_models <- foreach(disease = unique(train_disease_data$disease),
                         .packages = c("caret", "glmnet", "randomForest")) %dopar% {
  train_models(disease, train_disease_data, "disease")
}
names(disease_models) <- unique(train_disease_data$disease)

# 3. Phase Models
train_phase_data <- prepare_phase_data(train_data)
phase_models <- foreach(phase = unique(train_phase_data$phase),
                       .packages = c("caret", "glmnet", "randomForest")) %dopar% {
  train_models(phase, train_phase_data, "phase")
}
names(phase_models) <- unique(train_phase_data$phase)
...
## 4. Model Evaluation
We evaluate model performance using error metrics (RMSE, MAE) on the teasting data
```{r}
...
evaluate_models <- function(models, test_data_prepared, category_name) {
  ...
  for(target in names(models)) {
    ...
    x_test <- data.frame(
      ...
      month_factor = factor(target_test$month, levels = 1:12)
    )
    
    x_test_matrix <- model.matrix(
      ~ time_index + month_factor - 1, 
      data = x_test,
      contrasts.arg = list(month_factor = contrasts(factor(1:12)))
    )
    
    # Get predictions - model structure same as from parallel training
...
# Prepare test data using same preparation functions
...
cat("\n### Country Predictions:\n")
kable(determine_best_model(country_eval)$Model_Performance, format = "markdown")
cat("\n### Disease Distribution Predictions:\n")
kable(determine_best_model(disease_eval)$Model_Performance, format = "markdown")
cat("\n### Phase Pattern Predictions:\n")
kable(determine_best_model(phase_eval)$Model_Performance, format = "markdown")
```
...
- LASSO performs slightly better (RMSE: 7.55 vs 7.72)
- Suggests LASSO is more effective at capturing the country-specific patterns in trial counts

2. Disease Distribution:
- Both models perform similarly, but LASSO has a marginally lower RMSE (7.21 vs 7.32)
- Indicates disease trial patterns might be more linear in nature

3. Phase Patterns:
- LASSO again shows better performance (RMSE: 7.91 vs 8.00)
...
# Retrain LASSO models using parallel processing
retrain_final_model <- function(data, category_name) {
  ...
  final_models <- foreach(target = unique(prepared_data[[category_name]]),
  ...
  names(final_models) <- unique(prepared_data[[category_name]])
  return(final_models)
}
