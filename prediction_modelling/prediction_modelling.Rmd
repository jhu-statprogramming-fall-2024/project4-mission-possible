...
```{r}
# Single prediction function for dashboard use
predict_trials <- function(year, type, category) {
  # Input validation
  if(!type %in% c("disease", "country", "phase")) {
    stop("Type must be one of: disease, country, phase")
  }
  ...
  if(type == "disease") {
    new_data$disease <- category
    train_data <- splits$disease$train
  } else if(type == "country") {
    new_data$country <- category
    train_data <- splits$country$train
  } else if(type == "phase") {
    new_data$phase <- category
    train_data <- splits$phase$train
  }

  # Create model matrix
  combined_data <- rbind(train_data, new_data)
  X_new <- model.matrix(trial_count ~ ., data = combined_data)
  # Extract just the last row which corresponds to our new data
  X_new <- X_new[nrow(X_new), , drop = FALSE]
  
  # Make prediction using lasso_model from the models list
...
# Disease prediction
anxiety_pred <- predict_trials(2025, "disease", "Anxiety")
cat("Predicted trials for Anxiety:", anxiety_pred, "\n")

# Country prediction
us_pred <- predict_trials(2025, "country", "United.States")
cat("Predicted trials in United States:", us_pred, "\n")

# Phase prediction
phase1_pred <- predict_trials(2025, "phase", "PHASE1")
cat("Predicted Phase 1 trials:", phase1_pred, "\n")
```
### Documentation
